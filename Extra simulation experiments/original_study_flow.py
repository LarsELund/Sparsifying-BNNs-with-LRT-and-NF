...
from utils import BayesianLinearFlow, train
...
for i in range(0,30):
    ...
    X = np.genfromtxt('X_train_original' + str(i)+'.csv', delimiter=',',skip_header = 1)
    Y = np.genfromtxt('Y_train_original' + str(i) +'.csv', delimiter=',',skip_header = 1)
   ...
   # y =y / y.std()
    ...
    class BayesianNetwork(nn.Module):
        def __init__(self):
            ...
            self.l1 = BayesianLinearFlow(p, 1,num_transforms = 2) #one neuron = logistic regression
    ...
    predicted_alphas = np.zeros(shape=(num_models, 20))
    true_weights = np.genfromtxt('beta_original.csv', delimiter=',')
    ...
    np.savetxt('tpr_flow_binary_original' +str(i)  +'.txt',tpr, delimiter=',',fmt='%s')
    np.savetxt('fpr_flow_binary_original' + str(i)+'.txt',fpr, delimiter=',',fmt='%s')
