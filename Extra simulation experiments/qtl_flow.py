...
for i in range(20,30):
    ...
    X = np.genfromtxt('X_train_logit2' + str(i)+'.csv', delimiter=',',skip_header = 1)
    Y = np.genfromtxt('Y_train_logit2' + str(i) +'.csv', delimiter=',',skip_header = 1)
   ...
   # y =y / y.std()
    ...
    true_weights  = np.genfromtxt('beta_logits2.csv', delimiter=',')
    ...
    np.savetxt('tpr_flow_binary_logits2' +str(i)  +'.txt',tpr, delimiter=',',fmt='%s')
    np.savetxt('fpr_flow_binary_logits2' + str(i)+'.txt',fpr, delimiter=',',fmt='%s')
