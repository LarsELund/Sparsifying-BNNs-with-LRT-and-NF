...
for i in range(0,30):
    ...
    X = np.genfromtxt('X_train_comp' + str(i)+'.csv', delimiter=',',skip_header = 1)
    Y = np.genfromtxt('Y_train_comp' + str(i) +'.csv', delimiter=',',skip_header = 1)
   ...
   # y =y / y.std()
    ...
    true_weights = np.genfromtxt('beta_complicated.csv', delimiter=',')
    ...
    np.savetxt('tpr_flow_binary_comp' +str(i)  +'.txt',tpr, delimiter=',',fmt='%s')
    np.savetxt('fpr_flow_binary_comp' + str(i)+'.txt',fpr, delimiter=',',fmt='%s')
