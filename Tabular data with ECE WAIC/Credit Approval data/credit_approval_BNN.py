...
for i, (train_index, test_index) in enumerate(skf.split(X, y)):
    ...
    np.savetxt("credit_approval_train"+str(i)+".csv", train_dat.detach().cpu().numpy(),  
              delimiter = ",")
    
    np.savetxt("credit_approval_test"+str(i)+".csv", test_dat.detach().cpu().numpy(),  
              delimiter = ",")
...
current_dir = os.getcwd()
savepath = current_dir +'/results/credit_approval_BNN_metrics.txt'
    
np.savetxt(savepath, metrics_several_runs, delimiter=',')    

m = np.array(metrics_several_runs)
np.savetxt(current_dir +'/results/bnn_avg_metrics.txt',np.mean(m,axis = 0),delimiter = ',')
