...
for i, (train_index, test_index) in enumerate(skf.split(X, y)):
   ...
   # mu,std = X_train.mean(axis = 0), X_train.std(axis = 0) #standardize all columns
    ...
    np.savetxt("wine_train"+str(i)+".csv", train_dat.detach().cpu().numpy(),  
              delimiter = ",")
    
    np.savetxt("wine_test"+str(i)+".csv", test_dat.detach().cpu().numpy(),  
              delimiter = ",")
...
current_dir = os.getcwd()
savepath = current_dir +'/results/wine_ANN_metrics.txt'
    
np.savetxt(savepath, metrics_several_runs, delimiter=',')    
m = np.array(metrics_several_runs)
np.savetxt(current_dir +'/results/wineL2_avg_metrics.txt',m.mean(axis = 0),delimiter = ',')
